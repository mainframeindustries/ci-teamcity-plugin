...
public class BuildChainProcessor {
    ...
    private PipelineWebhook createPipelineWebhook(SBuild pipelineBuild, ChainMembership chainInfo) {
        ...
        // Combine TeamCity build tags and trigger information tags
        List<String> allTags = new ArrayList<>();
        
        // Add TeamCity build tags
        if (!pipelineBuild.getTags().isEmpty()) {
            allTags.addAll(pipelineBuild.getTags());
        }

        // Add trigger information tags
        List<String> triggerTags = extractTriggerTags(pipelineBuild);
        if (!triggerTags.isEmpty()) {
            allTags.addAll(triggerTags);
        }
        
        if (!allTags.isEmpty()) {
            pipelineWebhook.setTags(allTags);
        }
    ...
    private void logTriggeredByInfo(SBuild build, String buildType) {
        try {
            LOG.debug(format("--- %s: Build #%d '%s' ---", buildType, build.getBuildId(), buildName(build)));
            LOG.debug(format("  Build ID: %s", buildID(build)));
            LOG.debug(format("  Is Composite: %s", build.isCompositeBuild()));
            LOG.debug(format("  Start Date: %s", build.getStartDate()));
            LOG.debug(format("  Finish Date: %s", build.getFinishDate()));
            
            jetbrains.buildServer.serverSide.TriggeredBy triggeredBy = build.getTriggeredBy();
            if (triggeredBy != null) {
                LOG.debug("  TriggeredBy Information:");
                LOG.debug(format("    AsString: %s", triggeredBy.getAsString()));
                LOG.debug(format("    RawTriggeredBy: %s", triggeredBy.getRawTriggeredBy()));
                LOG.debug(format("    TriggeredDate: %s", triggeredBy.getTriggeredDate()));
                LOG.debug(format("    IsTriggeredByUser: %s", triggeredBy.isTriggeredByUser()));
                LOG.debug(format("    User: %s", triggeredBy.getUser() != null ? triggeredBy.getUser().getUsername() : "null"));
                LOG.debug(format("    TriggerId: %s", triggeredBy.getTriggerId()));
                LOG.debug(format("    Parameters: %s", triggeredBy.getParameters()));
            } else {
